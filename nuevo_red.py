...
FLASK_APP = "C:\\Users\\Usuario\Documents\\templates\\nuevo_red.py"
...
def search():
  ...
  if form.validate_on_submit():
    ...
    post = post.filter(Post.categories.like('%' + post_searched + '%'))
...
def account(id, username):
  ...
  if request.method == "POST":
    ...
    try:
       ...
       post.username = request.form['username']
       post.about = request.form['author']
       post.email = request.form['email']
       db.session.commit()
       flash('Cuenta actualizada!')
       return redirect(url_for('account', id = current_user.id, username = current_user.username))
  ...
  return render_template('account.html', form = form, image_file = image_file, post = post , postx = postx, pos = pos, poss = poss)
...
def delete_post(id):
  ...
  if post_delete.blog.id != current_user.id | post_delete.blog.id != 1: 
    abort(403)
...
def delete_account(id):
  ...
  if user_delete.id != current_user.id | current_user.id != 1: 
    abort(403)
...
def edit_post(id):
  ...
  if form.picturex.data:
       
       picture_file = save_picture(form.picturex.data)
       post.image_filex = picture_file
       db.session.commit()
...
def poster(id):
  ...
  post = Post.query.get_or_404(id)
  comenta = Comment.query.filter_by().all()
  form = commentx()
  ...
  if request.method == 'POST':
      ...
      text = request.form.get('comentario')
      comenta = Comment(username = username,  text = text, post_id = post.id)
  ...
  comenta = Comment.query.order_by(Comment.create_date)

  return render_template('post.html', form = form , comenta = comenta, post = post, post_id = post_id)   
...
@app.route('/categorias-ciencia', methods = ['GET', 'POST'])
@login_required
def categoria_ciencia():
  
  

 
 
  poste = Post.query.order_by(Post.categories)

  return render_template('categorias.html',  poste = poste)

@app.route('/categorias-deportes', methods = ['GET', 'POST'])
@login_required
def categorias_deportes():
  ...
  return render_template('categoriasx.html',  poste = poste)

@app.route('/categorias-tecnologia', methods = ['GET', 'POST'])
@login_required
def categorias_tecnologia():
  ...
  return render_template('categoriasxs.html',  poste = poste)

@app.route('/categorias-otros', methods = ['GET', 'POST'])
@login_required
def categorias_otros():
  ...
  return render_template('categoriasxss.html',  poste = poste)
...
def publish_post():
  ...
  if request.method == 'POST':
   ...
   post = Post(title = form.title.data , content = form.content.data , author = current_user.username , slug = form.slug.data,  vin_id = blog, categories = categoria )
  ...
  image_file = url_for('static', filename = 'imagenes/' + str(post.image_filex ))
        
  return render_template('publish_post.html', form = form, post = post, image_file = image_file)
