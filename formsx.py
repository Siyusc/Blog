...
class formx(FlaskForm):
    ...
    name =  StringField('FULL NAME',[validators.length(min=4,max=25, message='Ingresa un nombre y apellido valido' ),validators.DataRequired(message="El nombre es requerido")])
    username = StringField('USERNAME',[validators.length(min=4,max=25, message='Ingresa un username valido' ),validators.DataRequired(message="El username es requerido")])
...
class Postx(FlaskForm):
  ...
  picturex = FileField('Elige foto para tu post', validators=[FileAllowed(['jpg' , 'png'])])
